...
public:
	...
	IAAController(double aKp,double aKv, Model* modelCopy) : Controller(), kp( aKp ), kv( aKv)
	{
		...
		//_modelCopy->setAllControllersEnabled(false);
		//_modelCopy->getControllerSet().get(0).setDisabled(true);
		// getControllers
	...
	void computeControls(const SimTK::State& s, SimTK::Vector &controls) const
	{
		...
		//std::cout<<"copy model visual:"<<_modelCopy->getUseVisualizer()<<std::endl;
		//std::cout<<"copy model controllers  enabled:"<<_modelCopy->getAllControllersEnabled()<<std::endl;
		...
		//_modelCopy->getMultibodySystem().realize(si, SimTK::Stage::Acceleration);
	...
	std::cout<<"COM_desired_acc (b): "<<b<<std::endl;
		...
		*/
		...
		for(int i = 0; i < numMuscs; i++)
		{
			...
			//Vec3 udot_com_musc = _modelCopy->getMatterSubsystem().calcSystemMassCenterAccelerationInGround(s_solver);
	...
	//std::cout<<"A : "<<A;
		...
		for(int i = 0; i < numMuscs; i++)
		{
			//W(i,i) = 1 + 1000/listmaxfrcs[i]+0.1/(1.001-listacts[i])-0.1/(0.001+listacts[i]); //including the activations this way is not a very good soln, but it's a hack to make the optimization keep the activations below 1
...
int main()
{
	...
	try {
		...
		// Create the controller. Pass Kp, Kv values.6
